package org.jda.example.courseman.swclasses.cls4.modules.student;
...
viewDesc=@ViewDesc(
    ...
    view=View.class
     // SW4: view config customisation -> code customisation
...
controllerDesc=@ControllerDesc(
    controller=Controller.class,isDataFieldStateListener=true
    // SW4: controller config customisation -> code customisation
...
,containmentTree=@CTree(
    ...
    stateScope={Student.A_id, Student.A_name, 
        Student.A_modules // SW4: use modules instead of enrolments
        }
)
...
public class ModuleStudent {
  ...
  //SW4: 
  // - view config customisations: id, name, helpRequested, modules
  ...
  @AttributeDesc(label="Needs help?"
      ,alignX=AlignmentX.Center,isStateEventSource=true)
  private boolean helpRequested;
  
  @AttributeDesc(label="Enrols Into",
      type=JListField.class
      ,ref=@Select(clazz=CourseModule.class,attributes={"name"}),isStateEventSource=true
      ,width=100,
      height=5
  )
