...
public class TGC {
  ...
  public TGC(String name, String patternsConfigPath) {
    this.name = name;
  ...
  public void exec(Dom dom, JsonArray config)
    ...
    config.forEach(val -> {
      ...
      TPC transfProc = getTransformProcInstance(
  ...
  /**
   * @effects 
   *  Create and return a {@link TPC} instance defined for the pattern named <code>patternName</code>.
   *  Throws NotPossibleException if failed to create.
   */
  public TPC getTransformProcInstance(String patternName) throws NotPossibleException {
    // search in there
    ...
    while (it.hasNext()) {
      ...
      if (obj.getString(Params.pattern.name()).equals(patternName)) {
        ...
        try {
          return (TPC) Class.forName(transfProcCls).getConstructors()[0].newInstance();
