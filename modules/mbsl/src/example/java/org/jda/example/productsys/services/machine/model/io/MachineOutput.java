...
public class MachineOutput {
    ...
    @DAssoc(ascName = "Machine-assoc-MachineOutput", role = "rmo", ascType = AssocType.One2Many, endType = AssocEndType.Many, associate = @Associate(type = Machine.class, cardMin = 1, cardMax = 1))
    private Machine machine;

    @DAttr(name = "conveyor", type = Type.Domain, optional = false)
    @DAssoc(ascName = "Conveyor-assoc-MachineOutput", role = "rmo", ascType = AssocType.One2Many, endType = AssocEndType.Many, associate = @Associate(type = Conveyor.class, cardMin = 1, cardMax = 1))
    ...
    public MachineOutput(@AttrRef(value = "id") Integer id, @AttrRef(value = "machine") Machine machine, @AttrRef(value = "conveyor") Conveyor conveyor) throws ConstraintViolationException {
        this.id = genId(id);
        this.machine = machine;
        this.conveyor = conveyor;
    }
    ...
    public MachineOutput(@AttrRef(value = "machine") Machine machine, @AttrRef(value = "conveyor") Conveyor conveyor) throws ConstraintViolationException {
        this.id = genId(null);
        this.machine = machine;
        this.conveyor = conveyor;
    }
