...
public class StudClass implements Subscriber {
    ...
    public StudClass(@AttrRef("name") String name) {
        this(null, name);
    }

    // constructor to create objects from data source
    @DOpt(type=DOpt.Type.DataSourceConstructor)
    public StudClass(@AttrRef("id") Integer id, @AttrRef("name") String name) {
        this.id = nextID(id);
        this.name = name;
...
//        System.out.println(this + ".handleEvent(" + eventType + ", " + srcObj + ")");
        ...
        if (!data.stream().anyMatch(item -> item instanceof StudClass)) return;
