package org.jda.example.wordcount;
...
//      String file = fileName.indexOf(File.separator) > -1 ? fileName : WordCountDemo.class.getResource(fileName).getPath();
  ...
  static void createWordCountStream(final StreamsBuilder builder) {
    ...
    final KTable<String, Long> counts = source
        .flatMapValues(value -> {
          System.out.printf("%n*** data-item: %s%n ", value);
          return Arrays
            .asList(value.toLowerCase(Locale.getDefault()).split("\\W+"));
        } )
        .groupBy((key, value) -> value)
//        .count()
        .count().filter((word, count) -> 
        { System.out.printf("%n   (word,count): (%s,%d)%n", word, count);
          return true;
        });
